pipeline{
    ...
    parameters {
        ...
        string defaultValue: '----.yml', description: 'enter managed file name', name: 'vars_name', trim: true
    ...
    stages{
        stage('Check Out SCM'){
            steps{
                git url: 'e4s@vs-ssh.visualstudio.com:v3/e4s/E4S-PublicCloud/SystemTeam', credentialsId: 'p4s-engineering', branch: 'azure-config'
            }
        ...
        stage('AWS'){
            ...
            steps{
                ...
                configFileProvider([configFile(fileId: "${params.vars_name}", targetLocation: 'group_vars/single_server.yml')]) {
                }
                ...
                ansible-playbook aws-customer-server-config.yml --extra-vars @group_vars/single_server.yml
                """
            }
        }
        ...
        stage('Azure'){
            when{
             ...
             //get keys from vault
            environment {
            AZURE_CLIENT_ID = vault path: "secret/${params.subscription}/automation/azure/client_id", key: 'value', vaultUrl: 'http://10.4.0.11:8200', credentialsId: 'jenkinsreadvaulttoken'
            AZURE_SECRET = vault path: "secret/${params.subscription}/automation/azure/client_secret", key: 'value', vaultUrl: 'http://10.4.0.11:8200', credentialsId: 'jenkinsreadvaulttoken'
            AZURE_SUBSCRIPTION_ID = vault path: "secret/${params.subscription}/automation/azure/subscription_id", key: 'value', vaultUrl: 'http://10.4.0.11:8200', credentialsId: 'jenkinsreadvaulttoken'
            AZURE_TENANT = vault path: "secret/${params.subscription}/automation/azure/tenant_id", key: 'value', vaultUrl: 'http://10.4.0.11:8200', credentialsId: 'jenkinsreadvaulttoken'
            MEDIA_ACCESS_KEY= vault path: "secret/470172651571/automation/aws/access_key", key: 'value', vaultUrl: 'http://10.4.0.11:8200', credentialsId: 'jenkinsreadvaulttoken'
            MEDIA_SECRET_KEY= vault path: "secret/470172651571/automation/aws/secret_key", key: 'value', vaultUrl: 'http://10.4.0.11:8200', credentialsId: 'jenkinsreadvaulttoken'
            ...
            steps{
                ...
                export MEDIA_ACCESS_KEY=${env.MEDIA_ACCESS_KEY}
                export MEDIA_SECRET_KEY=${env.MEDIA_SECRET_KEY}
                ansible-playbook azure-customer-server-config.yml --extra-vars @group_vars/single_server.yml
                """
            }
        }
